...
void displayTabAnim(GtkWidget *widget, int dim, float tab[dim],int p,int j) {
    ...
    for (int i=0;i<dim;i++){
        ...
        GtkWidget *blueframe = gtk_frame_new(text);
        GtkWidget *whiteframe = gtk_frame_new(text);
        const char *cssClass = (p == 0 && i==j+1) || (p == 1 && i == j) || (p == 2 && i == j + 1) ? "blueframe" : "whiteframe";
        const char *css = g_strdup_printf("#%s { background-color: %s; }", cssClass, (p == 0 && i==j+1) || (p == 1 && i == j) || (p == 2 && i == j + 1) ? "#ADD8E6" : "white");
        GtkCssProvider *provider = gtk_css_provider_new();
        gtk_css_provider_load_from_string(provider, css);  //, -1, NULL);
        gtk_style_context_add_provider_for_display(gdk_display_get_default(), GTK_STYLE_PROVIDER(provider), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
        gtk_widget_set_name(blueframe, cssClass);
        gtk_widget_set_name(whiteframe, cssClass);
        if((p==1 || p==2) && (i==j || i==j+1)){
        ...
        sprintf(str, "%d", i);
        GtkWidget *ind = gtk_label_new(str);
        sprintf(str1, "Passe %d / %d \nO : Pas de permutation \nX : Permutation nécessaire\nLa bulle bleue transporte la plus grande valeur vers la position %d", iiteration+1,dim-1,dim-iiteration-1);
        GtkWidget *labelinfo = gtk_label_new(str1);
        gtk_grid_attach(GTK_GRID(Dgrid), ind, i + 5, 20, 1, 1);
        if((p==0 && i==j+1) || (p == 1 && i == j) || (p == 2 && i == j + 1)){
            gtk_grid_attach(GTK_GRID(Dgrid), blueframe, i + 5, 21, 1, 1);
        }else{
            gtk_grid_attach(GTK_GRID(Dgrid), whiteframe, i + 5, 21, 1, 1);
        }
        gtk_grid_attach(GTK_GRID(Dgrid), label, i+5, 22, 1, 1);
        gtk_grid_attach(GTK_GRID(Dgrid), labelinfo, 5, 15, 20, 1);
        g_free(text);
    }
}
...
gboolean delayed_permute (gpointer user_data) {
    ...
    displayTabAnim(GTK_WIDGET(Dgrid),dim, tab,2,jiteration);
    jiteration++;  
